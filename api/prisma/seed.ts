...
async function main() {
  ...
  const note1 = await prisma.note.create({
    data: {
      ...
      content: 'Just jotting down some random thoughts that crossed my mind today.',
      userId: defaultUser.id,
    },
  });
  ...
  const note2 = await prisma.note.create({
    data: {
      name: 'Task List',
      content: 'Need to do laundry, buy groceries, and finish that report for work.',
      userId: defaultUser.id,
    },
  });
  ...
  const attachedToNotebookNote = await prisma.note.create({
    data: {
      ...
      content: 'Today, I had a great conversation with an old friend and enjoyed a beautiful sunset. Grateful for these simple yet precious moments.',
      userId: defaultUser.id,
